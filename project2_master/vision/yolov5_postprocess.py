...
def filter_boxes(boxes, box_confidences, box_class_probs,
    ...
    # Step 1: filter by objectness >= threshold
    obj_mask = box_confidences.reshape(-1) >= obj_thresh
    boxes = boxes[obj_mask]
    box_confidences = box_confidences[obj_mask]
    box_class_probs = box_class_probs[obj_mask]

    if boxes.shape[0] == 0:
        return boxes, np.array([], dtype=np.int32), np.array([], dtype=np.float32)

    # Step 2: filter by class score >= threshold
    box_classes = np.argmax(box_class_probs, axis=-1)
    box_class_scores = np.max(box_class_probs, axis=-1)
    cls_mask = box_class_scores >= obj_thresh

    boxes = boxes[cls_mask]
    classes = box_classes[cls_mask]
    scores = box_confidences[cls_mask].reshape(-1) * box_class_scores[cls_mask]
...
def nms_boxes(boxes, scores, nms_thresh=None):
    ...
    while order.size > 0:
        ...
        iou = inter / (areas[i] + areas[order[1:]] - inter + 0.00001)
