...
size_t computeNv12Bytes(RK_U32 width, RK_U32 height) {
    size_t lumaBytes = 0U;
    if (width == 0U || height == 0U ||
        !tryMultiplySize(static_cast<size_t>(width), static_cast<size_t>(height), &lumaBytes)) {
        return 0U;
    }
...
int MppDecoder::readFrame(unsigned char **frame_data, int *width, int *height) {
    if (!frame_data || !width || !height || !mpi_ || !mpp_ctx_) {
        return -1;
    }
    ...
    while (true) {
        ...
        if (ret == MPP_OK && frame) {
            ...
            const size_t needed = computeNv12Bytes(w, h);
            size_t srcLumaBytes = 0U;

            if (needed == 0U || hor_stride < w || ver_stride < h ||
                !tryMultiplySize(static_cast<size_t>(hor_stride), static_cast<size_t>(ver_stride), &srcLumaBytes)) {
                printf("MppDecoder error: invalid frame geometry w=%u h=%u hs=%u vs=%u\n",
                       w, h, hor_stride, ver_stride);
                mpp_frame_deinit(&frame);
                return -1;
            }
            const size_t srcBytes = srcLumaBytes + (srcLumaBytes / 2U);
            if (mpp_buffer_get_size(buf) < srcBytes) {
                printf("MppDecoder error: short source buffer size=%zu required=%zu\n",
                       mpp_buffer_get_size(buf), srcBytes);
                mpp_frame_deinit(&frame);
                return -1;
            }
