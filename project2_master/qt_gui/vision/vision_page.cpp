...
void VisionPage::refresh() {
    ...
    fpsLabel_->setText(QString("FPS: %1").arg(QString::number(snapshot.fps, 'f', 1)));

    cameraLabel_->setText(QString("摄像头: %1").arg(snapshot.cameraOnline ? QStringLiteral("在线") : QStringLiteral("离线")));
    cameraLabel_->setStyleSheet(snapshot.cameraOnline ? "color: #32cd32;" : "color: #ff5a5a;");

    modelLabel_->setText(QString("模型: %1").arg(snapshot.modelLoaded ? QStringLiteral("已加载") : QStringLiteral("未加载")));
    ...
    if (!snapshot.frame.isNull()) {
        ...
        videoLabel_->setText(snapshot.errorMsg.isEmpty() ? QStringLiteral("等待摄像头...") : snapshot.errorMsg);
    }
